...
public class LockServiceTest extends SpringBaseTest {
    ...
    public void testLockAndRelease() throws Exception {
        UUID lockUuid = lockService.acquire(LockKeyUtils.getKey(1));
    ...
    public void testLockExpire() throws Exception {
        UUID lockUuid = lockService.acquire(LockKeyUtils.getKey(2));
        Assert.assertNotNull(lockUuid);
        TimeUnit.SECONDS.sleep(11);
        UUID secondlockUuid = lockService.acquire(LockKeyUtils.getKey(2));
        Assert.assertNotNull(secondlockUuid);
    }

    @Test
    public void testTryLockFailed() throws Exception {
        UUID uuid1 = lockService.acquire(LockKeyUtils.getKey(3));
        Assert.assertNotNull(uuid1);
        UUID uuid2 = lockService.acquire(LockKeyUtils.getKey(3), 8 * 1000);
    ...
    public void testTryLockSucc() throws Exception {
        UUID uuid1 = lockService.acquire(LockKeyUtils.getKey(4));
        Assert.assertNotNull(uuid1);
        UUID uuid2 = lockService.acquire(LockKeyUtils.getKey(4), 11 * 1000);
    ...
    public void testReleaseTimeoutLock() throws Exception {
        UUID uuid1 = lockService.acquire(LockKeyUtils.getKey(5), 1000, 1000);
        Assert.assertNotNull(uuid1);
        UUID uuid2 = lockService.acquire(LockKeyUtils.getKey(5), 2000, 1000);
        Assert.assertNotNull(uuid2);
